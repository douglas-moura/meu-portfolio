<template>   
    <section
        ...
        id="bloco-projeto"
        class="relative flex cursor-pointer m-0 mt-6 rounded-xl overflow-hidden bg-branco bg-cover bg-top justify-center items-center min-h-96 md:min-h-35rem lg:min-h-30rem 2xl:min-h-50rem"
    >
        <div id="bloco-conteudo" :class="['absolute h-full w-full backdrop-blur-lg transition-all duration-300 p-4 md:p-12', {'btn-oculto': !flipAtivo}]">
            <div class="text-branco transition-all duration-300 scale-100">
                <h5>{{ props.periodo }}</h5>
                <h2 class="min-h-20 2xl:text-4xl 2xl:my-4">{{ props.titulo }}</h2>
                <hr class="my-4 mx-0 border-laranja">
                <p class="mb-8 pr-4 text-xs md:text-base lg:text-xs 2xl:text-lg 2xl:mb-16">{{ props.descr }}</p>
                <span class="flex">
                    <Icon
                        v-for="tec in tecnicasProjeto" :key="tec.id"
                        :icon="tec.icone"
                        class="size-6 mr-4 md:size-10 md:mr-6 lg:size-6 2xl:size-10"
                    />
                </span>
            </div>
        </div>
