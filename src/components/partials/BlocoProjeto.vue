<template>   
    <section
        ...
        id="bloco-projeto"
        class="relative flex cursor-pointer m-0 mt-6 rounded-xl overflow-hidden bg-branco bg-cover bg-top justify-center items-center min-h-96 md:min-h-35rem lg:min-h-30rem"
    >
        <div id="bloco-conteudo" :class="['absolute h-full w-full backdrop-blur-lg transition-all duration-300 p-4 md:p-12', {'btn-oculto': !flipAtivo}]">
            <div class="text-branco transition-all duration-300 scale-100">
                ...
                <p class="mb-8 pr-4 text-xs md:text-base lg:text-xs">{{ props.descr }}</p>
                <span class="flex">
                    <Icon
                        v-for="tec in tecnicasProjeto" :key="tec.id"
                        :icon="tec.icone"
                        class="size-6 mr-4 md:size-10 md:mr-6 lg:size-6"
                    />
                </span>
            </div>
        </div>
